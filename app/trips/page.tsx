...
type SavedTrip = {
  ...
  budget: number;
  interests: string[];
  itinerary: { title?: string; currency?: string };
  updatedAt: string;
};
  ...
  return (
    <main className="trips-page">
      <nav className="nav shell">
        ...
        {user ? <button onClick={signOut} className="sign-in">Sign out</button> : <Link className="sign-in" href="/">Home <ArrowUpRight size={15}/></Link>}
      </nav>
      <section className="trips-hero">
        <div className="shell">
          <p className="eyebrow"><Sparkles size={14}/> YOUR TRAVEL LIBRARY</p>
          <h1>My trips.</h1>
          <p>Every itinerary you save, ready whenever the next adventure calls.</p>
        </div>
      </section>
      <section className="trips-library shell">
        ...
        {user === null && (
          <div className="account-prompt">
            <p className="eyebrow"><Compass size={14}/> YOUR TRIPS, IN ONE PLACE</p>
            <h2>Sign in to see where you&apos;re going.</h2>
            <p>Saved itineraries are private to your account and available across your devices.</p>
            <form onSubmit={submitAuth} className="library-auth">
              ...
              <label>Password<input type="password" autoComplete={authMode === "signIn" ? "current-password" : "new-password"} minLength={6} value={authPassword} onChange={(event) => setAuthPassword(event.target.value)} required/></label>
              {authMessage && <p className="auth-message">{authMessage}</p>}
              <button className="primary" disabled={authLoading}>{authLoading ? "Please wait…" : authMode === "signIn" ? "Sign in" : "Create account"}</button>
        ...
        {user && !loading && !error && trips.length === 0 && (
          <div className="empty-trips">
            ...
            <p>Plan your next escape, then save the itinerary here for later.</p>
            <Link href="/#planner" className="primary">Plan a trip <ArrowUpRight size={16}/></Link>
          </div>
        )}
        {user && !loading && !error && trips.length > 0 && (
              ...
              <Link href={`/trips/${trip.id}`} className="trip-card" key={trip.id}>
                <div className="trip-card-top">
                  ...
                  <div className="trip-card-actions">
                    <button type="button" className="delete-trip-btn" title="Delete trip" onClick={(e) => deleteTrip(e, trip.id)} disabled={deletingId === trip.id}>
                      {deletingId === trip.id ? <LoaderCircle className="spin" size={14}/> : <Trash2 size={14}/>}
                    </button>
                    <ArrowUpRight size={18}/>
                  </div>
                </div>
                <h2>{trip.itinerary?.title || `A trip to ${trip.destination}`}</h2>
                <p className="trip-dates"><CalendarDays size={15}/>{formatDate(trip.startDate)} – {formatDate(trip.endDate)}</p>
                <div className="trip-card-bottom">
                  <span>{formatCurrency(trip.budget, trip.itinerary?.currency || "USD")} budget</span>
                  <span>Updated {formatDate(trip.updatedAt)}</span>
                </div>
                <div className="trip-tags">
                  {trip.interests.slice(0, 3).map((interest) => <span key={interest}>{interest}</span>)}
                </div>
              </Link>
            ))}
          </div>
